 ...
 exports.register = function(app, dbCle, BASE_API_PATH, checkApiKeyFunction) {
     ...
     app.get(BASE_API_PATH + "/gdp/loadInitialData", function(request, response) {
        ...
        if (!checkApiKeyFunction(request, response)) return;
         ...
         dbCle.find({}).toArray(function(err, gdp) {
             ...
             console.log("INFO: Country gdp: " + gdp);
     ...
     app.get(BASE_API_PATH + "/gdp/:country:year", function(request, response) {
         ...
         if (isNaN(request.params.year.charAt(0))) {
             if (!country) {
                 ...
                 dbCle.find(queryCountry).toArray(function(err, results) {
                     if (err) {
                         console.error('WARNING: Error getting data from DB');
                         response.sendStatus(500); // internal server error
                     }
             ...
             if (!year) {
                 ...
                 dbCle.find(queryYear).toArray(function(err, results) {
                     if (err) {
                         console.error('WARNING: Error getting data from DB');
                         response.sendStatus(500); // internal server error
                     }
     ...
     app.get(BASE_API_PATH + "/gdp/:country/:year", function(request, response) {
         ...
         if (!country || !year) {
             ...
             dbCle.find(query).toArray(function(err, results) {
                 if (err) {
                     console.error('WARNING: Error getting data from DB');
                     response.sendStatus(500); // internal server error
                 }
                 ...
                 dbCle.find().toArray(function(err, gdp) {
                     if (err) {
                         ...
                         var gdpBeforeInsertion = gdp.filter((g) => {
                             return (g.country.localeCompare(newGdp.country, "en", {
                                 'sensitivity': 'base'
                             }) === 0);
             ...
             if (!updatedgdp.country || !updatedgdp.year) {
                 ...
                 dbCle.find({}).toArray(function(err, gdp) {
                     if (err) {
                         ...
                         var gdpBeforeInsertion = gdp.filter((g) => {
                             return (g.country.localeCompare(country, "en", {
                                 'sensitivity': 'base'
                             }) === 0);
                         });
