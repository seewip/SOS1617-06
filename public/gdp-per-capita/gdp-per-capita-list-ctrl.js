...
controller("GdpPerCapitaListCtrl", ["$scope", "$http", "$rootScope", function($scope, $http, $rootScope) {
    ...
    var refresh = $scope.refresh = function() {
         $http
            .get("../api/v1/gdp-per-capita" + modifier + "?" + "apikey=" + $rootScope.apikey + "&" + properties)
    ...
    $scope.addData = function() {
        $http
            .post("../api/v1/gdp-per-capita" + "?" + "apikey=" + $rootScope.apikey, $scope.newData)
            .then(function(response) {
                ...
                switch (response.status) {
                    case 409:
                        Materialize.toast('<i class="material-icons">error_outline</i> It already exists!', 4000);
                        break;                   
                    default:    
    ...
    $scope.delData = function(data) {
        $http
            .delete("../api/v1/gdp-per-capita/" + data.country + "/" + data.year + "?" + "apikey=" + $rootScope.apikey)
            .then(function(response) {
    ...
    $scope.delAllData = function() {
        $http
            .delete("../api/v1/gdp-per-capita" + "?" + "apikey=" + $rootScope.apikey)
            .then(function(response) {
    ...
    $scope.loadInitialData = function() {
        ...
        if ($scope.data.length == 0) {
            $http
                .get("../api/v1/gdp-per-capita/loadInitialData" + "?" + "apikey=" + $rootScope.apikey)
                .then(function(response) {
