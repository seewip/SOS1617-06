...
angular
    ...
    .controller("GdpRemoteKivaGraphCtrl", ["$scope", "$http", "$rootScope", function($scope, $http, $rootScope) {
        ...
        $http.get("https://api.kivaws.org/v1/lenders/search.json").then(function(response) {
            ...
            for (var i = 0; i < response.data.lenders.length; i++) {


                //array.push(Number(response.data.lenders[i].lender_id));
                //array.push(Number(response.data.lenders[i].name));
                $scope.uid.push(Number(response.data.lenders[i].image.id));
            }
